...
| 2026-09-20 | [메인 스레드가 곧 스텝이다](log/2026-09-20-k-the-main-thread-is-the-step.md) | ik를 perf로 보니 커널 스레드시간은 이미 같은 선(ik ~290 ms 대 우리 266 ms/스텝)이고 다른 것은 워커 이용률(75% 대 51%) — 메인 스레드의 직렬 구간이 곧 스텝 길이다. 그 절반이 할당·0 채우기: **스텝당 할당자 호출 14454번**(전부 토큰과 무관한 일 — 헤드·전문가 뷰, 메타데이터 키 `format!`, `ok_or`가 매번 복제하던 이름). 버퍼 프리리스트로 11463번, 속도는 무변화(래칫 게이트 `gate-alloc`로 남김, 나머지는 위임 라운드 진행 중). **SwiGLU를 `ggml_v_expf` 포트 8레인으로 +4.0%(62.37 → 64.84), ik와의 argmax 발산 집합 {24} → {}**; **`-C target-cpu=znver3` +4.8%(64.71 → 67.80)**, 비트 게이트 그대로. 기각: 내용 기반 입력 중복 제거(62.83 → 61.03). perf 함정 셋: IBS 귀속, `-D`는 해체 구간을 안 자른다(munmap 6%는 종료 비용이었다), 되돌린 실험의 바이너리를 프로파일함 |
