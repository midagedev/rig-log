...
| 2026-09-17 | [A Rust engine on the same card](log/2026-09-17-a-rust-engine-on-the-same-card.md) | the first step of the own-engine goal is a number, not a repo: mistral.rs 0.9.3 against ik_llama.cpp c10fbbcc on the same Qwen3.6-35B-A3B UD-Q6_K, same four prompts, same 512-token cap. Alone ik is **24 % faster** (130 against 105 tok/s client-side); at four streams mistral.rs is **90 % ahead** (283 against 149 aggregate) and its per-stream rate barely moves where ik's collapses 130 → 37.5, with engine prefill of four 234-token prompts at **104 ms against 3 357 ms** and TTFT p50 451 ms against 4 475. Not a Rust-kernel result: on CC 8.6 the Q6_K MoE path is candle's CUDA C kernels via FFI and cuTile needs CUDA 13.2 (box has 13.0), so this is scheduling and batching — and mistral.rs's +170 % at four streams contradicts the 09-16 expert-read arithmetic that explains ik's +13 %, which the next stream sweep has to settle. toktape cannot attach to an OpenAI-only server, so `tools/mrs/mrs-shim.py` fronts it with `/props`, `/apply-template` and llama-server `timings` taken from mistral.rs's own usage figures. Matching the runs found that **ik_llama.cpp ignores `chat_template_kwargs`**: every `--no-think` take in this repo was thinking, the hero take's `thinking off` is struck, and the recorder was asked to flag the contradiction |
