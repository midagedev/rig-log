...
- **로그는 하루 한 파일, 실험은 절이다** (2026-09-22 결정). `log/2026-MM-DD.md` 하나에 실험마다 `<a name="<slug>"></a>` + `## 제목` 절을 붙인다. 절은 질문 → 한 일 → 숫자 → 판명된 것·틀렸던 것 순서로 대체로 2.5 KB 안에 쓴다. 스텝별 연대기와 빌드 번호 이야기는 넣지 않는다 — 긴 상세가 필요하면 `docs/`에 쓰고 링크한다. 다른 로그는 `2026-MM-DD.md#<slug>`로 건다. 09-21 이전의 옛 실험별 파일명 대응은 `log/moved.tsv`, 원문은 커밋 `c27c3a7`.
