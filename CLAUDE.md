...
접속은 Tailscale 경유. ~~OpenAI 호환 API를 loopback에서 서빙, tailnet에 공개. `llm.service`가 GPU 둘을 쥐니 VRAM 작업은 중단부터 시작하고, 끝나면 다시 켜고 속도가 돌아왔는지 확인한다.~~ 2026-09-22 정정: `llm.service`는 inactive·disabled이고 야간 학습(webuta vocoder)은 09-21에 끝났다. **두 카드가 전부 우리 작업 것**이다 — 카드에 낯선 컴퓨트 프로세스가 있으면 양보할 대상이 아니라 조사할 일이다. ~~시간 수치는 3090에서 잰다(기준선 연속성).~~ 같은 날 저녁 정정: 3090이 부하 아래서 하루 두 번 버스에서 떨어져(Xid 79) **시간 수치는 A6000에서 잰다** — 기준선은 거기서 다시 잡았고, 그 전의 3090 수치와 한 표에 놓지 않는다. 3090은 게이트·빌드 카드, 300 W 캡.
