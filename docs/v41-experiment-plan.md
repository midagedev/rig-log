...
| Q4 | re-distil the draft against the served target (WKS-16): dump the target's greedy continuations for a few thousand prompts, fine-tune the tl37 draft head on them, re-run the twenty-prompt drafted pair | whether acceptance is bounded by the draft having learned the fp8 original rather than the Q3_K target it now serves against — the engram Q3→Q8 change moved acceptance only 58.0 → 58.9 % while target perplexity moved 6 %, which says the disagreement lives on the draft side and in the experts, not the tables | accepted/drafted before and after, same block, same placement; 60 → 70 % would be more than 10 % decode |
