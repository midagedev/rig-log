...
| 2026-09-25 | cuda-oxide | [#1329](https://github.com/NVlabs/cuda-oxide/pull/1329) — `cargo oxide`가 새 백엔드를 공유 캐시의 `.so` 위에 `fs::copy`로 덮어써(같은 inode를 truncate 후 재작성) 그 파일을 이미 올린 rustc가 SIGBUS로 죽는다; 병렬 트랙 둘에서 실제로 겪었다. 같은 디렉터리의 임시 파일로 복사한 뒤 rename. 수정 전 소스에서 "설치 전에 연 핸들이 옛 바이트를 읽는가" 테스트가 빨강, 수정 후 호스트 테스트 242개 녹색; 버린 CARGO_HOME에서 두 번째 `setup` 동안 `.so`를 mmap한 리더가 수정 전 3/3 SIGBUS, 수정 후 0/3. +154/−1, 1파일. | open |
