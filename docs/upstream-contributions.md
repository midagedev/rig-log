...
| 2026-09-13 | llama.cpp (V4.1 branch) | `resolve_fused_ops` disables a fused op for every layer when the probe finds it placed on a different device than the layer. For DeepSeek-V4's weightless hyper-connection *pre* node that happens at every GPU boundary on a two-card layer split: the node follows its inputs to the previous card, exactly where the unfused ops go. Fix: ask the layer device `ggml_backend_dev_supports_op` before reading placement as missing support (+10, commit e42d711e5 on the merged worktree, diff in the session scratchpad). Sibling finding: `-devd` with a single device aborts the scheduler for a DSpark draft, because the draft reads target hidden states on the other card — a clearer error would help. | candidate, not filed |
