...
| 2026-09-24 | ik_llama.cpp | [#2520](https://github.com/ikawrakow/ik_llama.cpp/pull/2520) — `unicode_tolower()`가 `std::lower_bound`로 찾는 `unicode_map_lowercase`가 `std::unordered_map`이라 정렬돼 있지 않았고, 키 1,433개 중 1,403개가 그대로 돌아왔다(`A`~`Z`까지). 두 케이스 표를 본가처럼 `std::initializer_list`로 선언한다(데이터는 이미 정렬돼 있다). `test-tokenizer-0 bert-bge` 18 실패 → 0(`Hello`가 `[UNK]`였다), Qwen3 `WE'REAL`이 본가와 같은 id로. `633e0617`(#689)부터의 결함이고 #1098·#2403에 증상이 있었다. +4/−4, 2파일. bloomery 세션이 냈다 — `gate-tokenizer`의 qwen3moe 빨강(`'RE`)이 참조 쪽 결함이었다. | **merged 2026-09-24**(ikawrakow) |
| 2026-09-24 | ik_llama.cpp | [#2527](https://github.com/ikawrakow/ik_llama.cpp/pull/2527) — `-mqkv 1 -muge 1`이면 합친 텐서와 그 뷰(`wq/wk/wv`, `ffn_gate/up_exps`)가 전부 `tensors_by_name`에 있어 `llama_model_size`·`llama_model_n_params`가 합친 몫을 두 번 센다; llama-bench 표와 서버 `/v1/models` 메타가 같은 함수를 쓴다. `llm_load_tensors`의 repack 루프처럼 `view_src`가 있는 텐서를 건너뛴다. Qwen3-30B-A3B Q4_K_M(GGUF 헤더 18,550,716,416바이트·30,532,122,624파라미터)에서 수정 전 27.65 GiB·50.34 B, 수정 후 17.28 GiB·30.53 B로 플래그 없을 때와 같다. +2/−0, 1파일. | **merged 2026-09-24**(ikawrakow) |
| 2026-09-24 | ik_llama.cpp | [#2528](https://github.com/ikawrakow/ik_llama.cpp/pull/2528) — `unicode_regex_split`의 접힌 SYMBOL 클래스 ASCII 목록에 `~`가 빠져 `\p{S}`가 `~`에 안 맞고, `[\p{P}\p{S}]`를 쓰는 프리토크나이저(deepseek-v3 등)가 `~/`를 `~`와 `/`로 갈랐다. `\x7E` 한 글자, ggml-org/llama.cpp#26972과 같다. V4.1-Flash 어휘로 `` `~/` directory``가 `3608, 96, 17, 66, 20289` → `3608, 71520, 66, 20289`(HF tokenizers와 같음); 75k·779k 토큰 파일 둘이 수정 후 HF와 전부 같고 수정 전에는 76번째·10,968번째에서 첫 차이. +1/−1. bloomery 세션이 냈다 — engram 코퍼스 다섯 개가 수정된 토크나이저로 재면 id 수가 줄고(code 778,951 → 778,925 등) 코퍼스는 재생성하지 않았다(뜨거운 목록·타이밍 기준선이 그 id를 먹는다). | **merged 2026-09-24**(ikawrakow) |
| 2026-09-24 | ik_llama.cpp | [#2522](https://github.com/ikawrakow/ik_llama.cpp/pull/2522) — #2438의 나머지 절반(#2455에서 예고). V4.1 DSpark 드래프트가 `check_tensor_dims: tensor 'output_hc_base.weight' not found`로 적재되지 않았다 — V4.1에는 `output_hc_*` 헤드가 없고 드래프트도 몸체를 따른다(지연 hyper-connection 혼합, 마지막 FFN 혼합으로 출력 접기, 헤드별 q norm 없음). `output_hc_base.weight`가 없는 DSV4 드래프트는 그 규칙으로 적재하고 V4 드래프트는 그대로; 두 번째 커밋은 드래프트 블록 토큰이 서로와 블록 시작에 고정된 창 하나를 보게 한다(참조 구현과 같음). +56/−14, 5파일. 드래프트로 열어 두었다가 다른 기여자가 V4·V4.1 드래프트 둘 다 시험한 뒤 ready로 돌렸다; 공개 파일 Q3_K_M 재측정과 V4 경로 등식은 [기록](../log/2026-09-25.md#ik-dspark-not-lossless)(수락 58.1 %/43.5 %, 탐욕 출력은 보존되지 않는다 — PR과 무관한 ik DSpark 자체의 성질). 메인테이너 제안대로 어서트 둘과 `llama-dflash.cpp` 마스크 조건 둘을 고쳤다. | **merged 2026-09-25**(ikawrakow) |
