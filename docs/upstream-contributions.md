...
| 2026-09-15 | exllamav3 | MTP drafting with CPU-offloaded experts (`-mcs`/`-mcl`) uses the drafter's `default_draft_size` 3, and on GLM-5.3-Flash at 4 bpw with 103 of 288 experts resident that loses 14 % against no draft (20.6 vs 23.3 tok/s, 47 % acceptance) where depth 1 gains 9 % (25.4, 77 %); the verify step is RAM-bandwidth-bound so only one drafted position pays. Prior-art search and a reproducer on a second placement still to do before filing; the fix may be a depth default keyed to offload mode or a docs line. [Write-up](../log/2026-09-15-glm-5.3-flash-first-run.md). | candidate, not filed |
