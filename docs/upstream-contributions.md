...
| 2026-09-17 | mistral.rs | **not filed** - the CPU GGUF MoE path dequantizes a block's entire expert stack to F32 and rebuilds an `UnquantLinear` on every forward, one line above where #2430 landed, so an offloaded layer costs a flat **442 ms per token** (measured 443.7 / 442.2 / 441.5 ms per layer at 1, 4 and 10 blocks on the host) against 0.20 ms for ik_llama.cpp, which keeps the matmul on the GPU and reads only the 8 of 256 routed experts from pinned host memory. Nothing in the path uses the sparsity. Two candidate shapes: cache the dequantized stack, or gather the routed experts before dequantizing. Held because a rate is not a diagnosis - 442 ms is consistent with the dequantize and with other things, and the next step is a profile of one offloaded layer that attributes the milliseconds. Same rule that holds ik's batch-2 decode finding. | unfiled |
