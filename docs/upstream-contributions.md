...
|---|---|---|---|
| 2026-09-21 | ik_llama.cpp | **후보(측정만, 원인 미확인)** — `llama_kv_cache_clear` 뒤에도 앞 시퀀스가 생성한 토큰이 다음 프롬프트의 로짓을 바꾼다. V2-Lite Q3_K_M, 토큰 하나씩 디코드(M=1): 같은 7토큰 프롬프트를 두 번 넣고 사이에 1토큰을 생성하면 두 번째 답이 1191(30.22)에서 245(29.23)로, 생성 없이는 둘 다 1191. **CPU 백엔드(CUDA 숨김)에서도 같다**(188/1191 → 188/245). 컨텍스트 크기(`-c 512`/544)는 무관(바이트 동일). 재현자는 bloomery `tools/ref/argmax_ref.cpp --gen 1`에 같은 프롬프트 2행. 우리 참조 파일 `greedy-ik-cuda-32.tsv`의 행 1..32가 이것으로 오염돼 있었다. 다음: ik의 `llama_kv_cache_clear`·마스크 빌드 독해, ik 라이브러리만 링크한 최소 C++ 재현자, mainline llama.cpp 동일 여부. [기록](../log/2026-09-21-i-the-first-token-came-out-at-0918.md). | candidate, not filed |
