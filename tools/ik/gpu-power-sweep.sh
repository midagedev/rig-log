...
CAPS=${*:-$(nvidia-smi -i 0 --query-gpu=power.default_limit --format=csv,noheader,nounits | cut -d. -f1)}
TAKE=${TAKE:-/home/user/ik-vram-take.sh}
OUT=${OUT:-/home/user/ik-vram}; mkdir -p "$OUT"
DEF=$(nvidia-smi -i 0 --query-gpu=power.default_limit --format=csv,noheader,nounits | cut -d. -f1)
...
restore(){ nvidia-smi -i 0 -pl "$DEF" >/dev/null 2>&1
  echo "$(date +%H:%M:%S) cap restored to $(nvidia-smi -i 0 --query-gpu=power.limit --format=csv,noheader,nounits) W"; }
trap restore EXIT
for pl in $CAPS; do
  nvidia-smi -i 0 -pl "$pl" >/dev/null || { echo "could not set cap $pl W (root? within min/max?)"; exit 1; }
  echo "=== cap now $(nvidia-smi -i 0 --query-gpu=power.limit --format=csv,noheader,nounits) W"
