#!/bin/bash
# qwen38-take.sh <tag> — Qwen3.8-Flash-Next on mainline llama.cpp, two cards + RAM, one toktape take.
...
set -u
TAG=$1; OUT=/home/user/qwen38/$TAG; LEASE=/home/user/gpu-lease; PORT=8013; URL=http://127.0.0.1:$PORT
B=${B:-/home/user/llama.cpp/build/bin/llama-server}; RUNS=/home/user/toktape-runs; TOKTAPE=${TOKTAPE:-/home/user/toktape-hero-0f88bd3}
M=${M:-/models/Qwen3.8-Flash-Next/Qwen3.8-Flash-Next-UD-Q4_K_XL-00001-of-00004.gguf}
...
SPID=""; SENT=QWEN38; LEASE_TAG="qwen38-$TAG"
finish(){ rc=$1
...
setsid nohup "$B" -m "$M" --alias Qwen3.8-Flash-Next -c ${CTX:-32768} -ngl 99 -fa on -t ${THREADS:-32} -b 2048 -ub ${UB:-512} \
