#!/bin/bash
# run6: mixed-prompt steady state (bench2 alternates 5 prompts), MTP draft, thread count. Sentinel EXL3_RUN6_DONE.
say(){ echo "$(date +%H:%M:%S) $*"; }
...
arm(){ tag=$1; b=$2; shift 2; say "== $tag ($b): $*  io $(grep some /proc/pressure/io | cut -d" " -f2)"; t0=$(date +%s)
  ...
  sleep 10; }
arm mix185 exl3-bench2.py -gs 44,21 -mcs 185 -mct 32 --runs 10
arm mtp185 exl3-bench.py -gs 44,21 -mcs 185 -mct 32 -mtp --runs 4
arm mtp195 exl3-bench.py -gs 44,21 -mcs 195 -mct 32 -mtp --runs 4
arm t48_185 exl3-bench.py -gs 44,21 -mcs 185 -mct 48 --runs 4 --no_prefill
say EXL3_RUN6_DONE
