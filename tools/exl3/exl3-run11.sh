#!/bin/bash
# run11: routing profile (mixed 10 runs, no sweeps) -> static placement from stats vs dynamic; CPU worker knobs (slots, staging threads); faster swap interval. Sentinel EXL3_RUN11_DONE.
say(){ echo "$(date +%H:%M:%S) $*"; }
until grep -q "EXL3_RUN10_DONE" /home/user/exl3-run10.log; do sleep 15; done
...
arm(){ tag=$1; b=$2; shift 2; say "== $tag ($b) [$E]: $*  io $(grep some /proc/pressure/io | cut -d" " -f2)"; t0=$(date +%s)
  ...
  sleep 10; }
E="EXL3_MOE_CPU_SWAP=0 EXL3_MOE_CPU_SPLIT_STATS=$HOME/exl3-stats-mixed.json" arm staticfix exl3-bench6.py -gs 44,21 -mcs 185 -mct 32 -mtp --draft_n 1 --runs 5 --no_prefill --patch_defer
say "answers tail (staticfix):"; tail -n 5 $HOME/exl3-ans2.txt | cut -c1-140
say EXL3_RUN11_DONE
