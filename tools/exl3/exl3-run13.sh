#!/bin/bash
# run13: routing profile (mixed 10 runs, no sweeps) -> static placement from stats vs dynamic; CPU worker knobs (slots, staging threads); faster swap interval. Sentinel EXL3_RUN13_DONE.
say(){ echo "$(date +%H:%M:%S) $*"; }
...
say EXL3_RUN13_DONE
E="EXL3_MOE_CPU_SWAP=0 EXL3_MOE_CPU_SPLIT_STATS=$HOME/exl3-stats-mixed.json" arm staticpatch250 exl3-bench5.py -gs 44,21 -mcs 250 -mct 32 -mtp --draft_n 1 --runs 3 --no_prefill
say EXL3_RUN13_DONE
