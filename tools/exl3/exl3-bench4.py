# exllamav3 bench v4 = v2 (alternating prompts) + --draft_n / --dyn_draft. alternates prompts run to run (essay / coding / explanation) so the dynamic placement is measured on a mixed stream, not one text's routing histogram. Greedy 400-token decode x N + one 11k-token prefill, timings from the Job results.
