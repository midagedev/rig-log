# exllamav3 bench, v2: alternates prompts run to run (essay / coding / explanation) so the dynamic placement is measured on a mixed stream, not one text's routing histogram. Greedy 400-token decode x N + one 11k-token prefill, timings from the Job results.
...
def main():
    ...
    gen = Generator(model = model, cache = cache, tokenizer = tokenizer, draft_model = draft_model, draft_cache = draft_cache, max_batch_size = 1)
