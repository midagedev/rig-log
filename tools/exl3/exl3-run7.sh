#!/bin/bash
# run7: mixed-prompt steady state (bench2 alternates 5 prompts), MTP draft, thread count. Sentinel EXL3_RUN7_DONE.
say(){ echo "$(date +%H:%M:%S) $*"; }
...
arm(){ tag=$1; b=$2; shift 2; say "== $tag ($b): $*  io $(grep some /proc/pressure/io | cut -d" " -f2)"; t0=$(date +%s)
  ...
  sleep 10; }
arm mtp185n1 exl3-bench3.py -gs 44,21 -mcs 185 -mct 32 -mtp --draft_n 1 --runs 4 --no_prefill
arm mtp185n2 exl3-bench3.py -gs 44,21 -mcs 185 -mct 32 -mtp --draft_n 2 --runs 4 --no_prefill
arm mtp185dyn exl3-bench3.py -gs 44,21 -mcs 185 -mct 32 -mtp --dyn_draft --runs 4 --no_prefill
arm mtp185n2mix exl3-bench3.py -gs 44,21 -mcs 185 -mct 32 -mtp --draft_n 2 --runs 4
say EXL3_RUN7_DONE
