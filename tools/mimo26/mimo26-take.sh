#!/bin/bash
# mimo26-take.sh <tag> — MiMo-V2.6-Flash-RL (MXFP4, ggml-org) on mainline llama.cpp, two cards + RAM, one toktape take.
...
set -u
TAG=$1; OUT=/home/user/mimo26/$TAG; LEASE=/home/user/gpu-lease; PORT=8014; URL=http://127.0.0.1:$PORT
B=${B:-/home/user/llama.cpp/build/bin/llama-server}; RUNS=/home/user/toktape-runs; TOKTAPE=${TOKTAPE:-/home/user/bin/toktape-main}
M=${M:-/models/MiMo-V2.6-Flash-RL/MiMo-V2.6-Flash-RL-MXFP4-00001-of-00002.gguf}
...
SPID=""; SENT=MIMO26; LEASE_TAG="mimo26-$TAG"
finish(){ rc=$1
...
say "OT=$OT"
DRAFT=(); [ -z "$D" ] || DRAFT=(-md "$D" --spec-type draft-mtp --spec-draft-n-max ${NMAX:-5} ${OTD:+-otd "$OTD"})
. /home/user/gpu-order.env
setsid nohup "$B" -m "$M" --alias MiMo-V2.6-Flash -c ${CTX:-32768} -ngl 99 -fa on -t ${THREADS:-32} -b 2048 -ub ${UB:-512} \
